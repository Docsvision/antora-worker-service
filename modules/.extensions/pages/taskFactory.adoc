...
* `CreateWorkerTask(IMessage, IWorkerTask)` -- должен создавать _Задачу_ по переданному _Сообщению_, если требуется, с учётом последнего выполненного _Задания_ из группы текущего _Задания_.
...
+
[#STask]
.. Создайте раздел с произвольным именем в реестре ОС, например `{hklm-dv}\WorkerService\Tasks\STask`.
+
...
+
.. Создайте раздел с произвольным именем в реестре ОС, например `{hklm-dv}\WorkerService\Roles\RoleWithSTask`.
+
...
+
.. Создайте раздел с произвольным именем в реестре ОС, например `{hklm-dv}\WorkerService\Roles\WorkerProcesses\WpSTask`.
+
