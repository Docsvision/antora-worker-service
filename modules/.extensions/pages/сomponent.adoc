...
+
. Создайте раздел с произвольным именем в реестре ОС, например `{hklm-dv}\WorkerService\Components\SomeComponent`.
+
...
+
.. Создайте раздел с произвольным именем в реестре ОС, например `{hklm-dv}\WorkerService\Roles\RoleWithBestComponent`.
+
...
+
. Создайте раздел с произвольным именем в реестре ОС, например `{hklm-dv}\WorkerService\Roles\WorkerProcesses\WpBestComponent`.
+
...
. Создайте папку `C:\logs\` и предоставьте полные права на данную папку учётной записи службы _WorkerService_.
