...
Если для создания _Подключения_ нужен доступ к link:Services.md[сервисам API Службы], в классе расширения нужно реализовать интерфейс `IInitializeWithServiceProvider`. См. link:GetServiceProvider.md[Получение поставщика сервисов в расширении].
...
Для получения подключения, предоставляемого какой-либо _Фабрикой подключений_, необходимо обращаться к link:Services.md[сервису] «Пул подключений» (тип `IConnectionPool`):
