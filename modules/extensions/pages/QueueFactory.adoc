...
В данном случае никакая _Задача_ не создаётся – только обрабатывается _Сообщение_. Для того, чтобы по _Сообщению_ создавалась _Задача_ нужно реализовать расширение link:WorkerTaskFactory.md[Фабрика задач], у которой в списке поддерживаемых типов _Сообщений_ (`IWorkerTaskFactory.MessageTypes`) должен быть идентификатор типа _Сообщения_ `FSMessage` (`IMessage.TypeId`).