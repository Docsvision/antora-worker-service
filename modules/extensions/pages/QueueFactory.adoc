...
. Создайте раздел `HKLM\SOFTWARE\{dv}\WorkerService\5.5\Queues\FSQueue` ("FSQueue" -- выбрано произвольно) в реестре ОС.
...
image:queueRegistry.png[]

. Создайте `Роль рабочего процесса`, через которую РП будет получать расширение `FSQueueFactory`:

. Создайте раздел `HKLM\SOFTWARE\{dv}\WorkerService\5.5\Roles\RoleWithFSQueue` ("RoleWithFSQueue" -- выбрано произвольно) в реестре ОС.
...
image:queueRoleRegistry.png[]

. Зарегистрируйте экземпляр РП с _Ролью_ "RoleWithFSQueue".

. Создайте раздел `HKLM\SOFTWARE\{dv}\WorkerService\5.5\Roles\SOFTWARE\{dv}\WorkerService\5.5\WorkerProcesses\WpFSQueue` ("WpFSQueue" -- выбрано произвольно) в реестре ОС.
