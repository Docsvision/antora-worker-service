...
Описание данных интерфейсов приведено в пункте link:ObjectTypes.md[Типы объектов].
...
Если для выполнения функций _Компонента_ нужен доступ к link:Services.md[сервисам API Службы], в классе расширения нужно реализовать интерфейс `IInitializeWithServiceProvider`. См. link:GetServiceProvider.md[Получение поставщика сервисов в расширении].
