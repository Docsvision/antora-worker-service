...
public class DocumentWasSent : IWorkerTask
{
    ...
    public void Work()
    {
        // Получения пользовательской сессии {dv}
...
* В класс `SomeTaskFactory` добавлен интерфейс `IInitializeWithServiceProvider` для возможности передачи в создаваемые _Задания_ xref:GetServiceProvider.adoc[Поставщика сервисов] (используется в Задача для получения Подключения к {dv}).
...
. Создайте раздел `HKLM\SOFTWARE\{dv}\WorkerService\5.5\Tasks\STask` ("STask" -- выбрано произвольно) в реестре ОС.

. В созданном разделе укажите полное имя класса `SomeTaskFactory` в параметре по умолчанию.

image:taskRegistry.png[]

. Создайте `Роль рабочего процесса`, через которую РП будет получать расширение `SomeTaskFactory`:

. Создайте раздел `HKLM\SOFTWARE\{dv}\WorkerService\5.5\Roles\RoleWithSTask` ("RoleWithSTask" -- выбрано произвольно) в реестре ОС.
...
image:taskRoleRegistry.png[]

. Зарегистрируйте экземпляр РП с _Ролью_ "RoleWithSTask".

. Создайте раздел `HKLM\SOFTWARE\{dv}\WorkerService\5.5\Roles\SOFTWARE\{dv}\WorkerService\5.5\WorkerProcesses\WpSTask` ("WpSTask" -- выбрано произвольно) в реестре ОС.
