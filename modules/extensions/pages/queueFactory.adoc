...
[%collapsible]
====
* `TryDequeue` -- данный метод должен возвращать следующую необработанную _Группу сообщений_. Сообщения группы должны быть заблокированы от возможности повторной обработки (в т. ч. другими экземплярами РП).
+
...
IMPORTANT: Реализация метода `TryDequeue` должна гарантировать защиту от повторной обработки Сообщения, загруженного _Очередью сообщений_.
====
...
+
.. Создайте раздел с произвольным наименованием в реестре ОС, например `HKLM\SOFTWARE\{dv}\WorkerService\5.5\Queues\FSQueue`.
+
...
+
. Создайте раздел с произвольным именем в реестре ОС, например `HKLM\SOFTWARE\{dv}\WorkerService\5.5\Roles\RoleWithFSQueue`.
+
...
+
. Создайте раздел с произвольным именем в реестре ОС, например `HKLM\SOFTWARE\{dv}\WorkerService\5.5\Roles\SOFTWARE\{dv}\WorkerService\5.5\WorkerProcesses\WpFSQueue`.
+
