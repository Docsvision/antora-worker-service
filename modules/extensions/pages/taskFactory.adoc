...
----
<.> Получения пользовательской сессии {dv}. РП должно быть доступно _Подключение_ с типом `UserSession`.
...
+
[#STask]
.. Создайте раздел с произвольным именем в реестре ОС, например `HKLM\SOFTWARE\{dv}\WorkerService\5.5\Tasks\STask`.
+
...
+
.. Создайте раздел с произвольным именем в реестре ОС, например `HKLM\SOFTWARE\{dv}\WorkerService\5.5\Roles\RoleWithSTask`.
+
...
+
.. Создайте раздел с произвольным именем в реестре ОС, например `HKLM\SOFTWARE\{dv}\WorkerService\5.5\Roles\SOFTWARE\{dv}\WorkerService\5.5\WorkerProcesses\WpSTask`.
+
