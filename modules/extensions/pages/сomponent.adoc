...
* `Start` -- в методе содержится реализация требуемой функциональности расширения, например обработка карточки {dv}. Данная функция будет вызвана один раз при запуске (перезапуске) РП.
...
РП не завершит свою работу, если хотя бы у одного из его _Компонентов_ `Stopped` в значении `*FALSE*`.
====
...
<.> Вызывается при запуске _Компонента_ (при запуске РП).
<.> Нужно указать, что _Компонент_ запущен (не остановлен).
<.> Вызывается при остановке _Компонента_ (при остановке РП).
...
+
. Создайте раздел с произвольным именем в реестре ОС, например `HKLM\SOFTWARE\{dv}\WorkerService\5.5\Components\SomeComponent`.
+
...
+
.. Создайте раздел с произвольным именем в реестре ОС, например `HKLM\SOFTWARE\{dv}\WorkerService\5.5\Roles\RoleWithBestComponent`.
+
...
+
. Создайте раздел с произвольным именем в реестре ОС, например `HKLM\SOFTWARE\{dv}\WorkerService\5.5\Roles\SOFTWARE\{dv}\WorkerService\5.5\WorkerProcesses\WpBestComponent`.
+
